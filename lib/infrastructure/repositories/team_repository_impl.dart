...
class TeamRepositoryImpl implements TeamRepository {
  ...
  Future<Either<TeamFailure, Team>> getById(String teamId) async {
    try {
      ...
      debugPrint('📥 [TeamRepository] getById: $teamId');
  ...
  Stream<Either<TeamFailure, List<Team>>> watchAllTeams() async* {
    debugPrint('🎯 [TeamRepository] watchAllTeams STREAM STARTED');
    ...
    yield* teamsCollection.snapshots()
      ...
      debugPrint('📥 [TeamRepository] watchAllTeams EVENT #$eventCount: ${snapshot.docs.length} teams');
      try {
        final teams = snapshot.docs
            .map((doc) => TeamModel.fromFirestore(doc).toDomain())
            .toList();
